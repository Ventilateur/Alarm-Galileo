...
void MyLcd::writeMsg(std::string msg) {
	for (std::string::size_type i = 0; i < msg.size(); ++i) {
		MyLcd::writeData(myLcd, msg[i]);
	}
}

void MyLcd::setPos(int row, int col) {
...
void MyLcd::writeCmd(mraa_i2c_context ctx, uint8_t val) {
	uint8_t data[2] = {LCD_CMD, val};
	mraa_i2c_address(ctx, LCD_CTRL_ADR);
	mraa_i2c_write(ctx, data, 2);
}

void MyLcd::writeReg(mraa_i2c_context ctx, int devAdr, uint8_t adr, uint8_t val) {
